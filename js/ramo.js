...
class Ramo {
    ...
    getDisplayCredits() {
        if (this.malla.sct) {
            ...
            return this.getStandardCredits()
        }
    }
    ...
    draw(canvas, posX, posY, scaleX, scaleY) {
        ...
        this.ramo.append("title").text(
            "Ramo " + this.sigla + ", " + this.name + ". Este ramo tiene " + this.getStandardCredits() + " créditos y " +
    ...
    // Auto explanatorio
    approveRamo() {
        // Cycle status: null -> approved -> failed -> pending -> null
        if (!this.status) {
            this.status = "approved";
            this.approved = true;
            this.malla.approveSubject(this);
            d3.select("#" + this.sigla).classed("status-approved", true);
        } else if (this.status === "approved") {
            this.status = "failed";
            this.approved = false;
            this.malla.deApproveSubject(this);
            d3.select("#" + this.sigla).classed("status-approved", false).classed("status-failed", true);
        } else if (this.status === "failed") {
            this.status = "pending";
            d3.select("#" + this.sigla).classed("status-failed", false).classed("status-pending", true);
        } else {
            this.status = null;
            d3.select("#" + this.sigla).classed("status-pending", false);
        }
    }
