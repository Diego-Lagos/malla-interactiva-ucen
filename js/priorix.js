class Priorix extends SemesterManager {
    constructor(malla, card) {
        ...
        // {Standard : {1 : numero, 2: otroNumero, ...}}, SCT : {...}}
    ...
    addSubject(subject) {
        ...
        this.totalCredits['Standard'] = this.totalCredits['Standard'] + subject.getStandardCredits()
        ...
        Object.keys(this.selectedPerSemester).forEach(semester => {
            if (semester !== this.semester) {
                ...
                if (found !== -1) {
                    ...
                    if (semester < this.semester) {
                        subject.approveRamo()
                        this.totalCredits["Standard"] -= subject.getStandardCredits()
                        this.totalCredits["SCT"] -= subject.getSCTCredits()

                        let grade = this.subjectGrades[semester][subject.sigla]
                        let scoreToDeleteStandard = grade * subject.getStandardCredits()
                        ...
                        for (semester; semester < this.semester; semester++) {
                            if (grade > 54) {
                                this.totalApprovedCredits["Standard"][semester] -= subject.getStandardCredits()
                                this.totalApprovedCredits["SCT"][semester] -= subject.getSCTCredits()
                            }
                            this.prevSemesterSums["Standard"][semester] -= scoreToDeleteStandard
    ...
    displaySubject(subject) {
        ...
        subjectGrade.append('div')
            ...
            .append('span')
            .attr('class', 'input-group-text')
            .text('x ' + subject.getStandardCredits() + ' Créditos | ' + subject.getSCTCredits() + ' SCT');
    ...
    nextSemester() {
        ...
        this.prevSemesterSums["Standard"][this.semester] = this.currentSemesterSum["Standard"]
    ...
    calculate() {
        if (this.calculationsEnabled) {
            let currentApprovedCreditsStandard, currentApprovedCreditsSCT
            let currentSemesterSumStandard, currentSemesterSumSCT
            let semesterGrades = {}
            if (this.semester !== 1) {
                currentApprovedCreditsStandard = this.totalApprovedCredits["Standard"][this.semester - 1]
                currentApprovedCreditsSCT = this.totalApprovedCredits["SCT"][this.semester - 1]

                currentSemesterSumStandard = this.prevSemesterSums["Standard"][this.semester - 1]
                currentSemesterSumSCT = this.prevSemesterSums["SCT"][this.semester - 1]
            } else {
                currentApprovedCreditsStandard = 0
                currentApprovedCreditsSCT = 0
                currentSemesterSumStandard = 0
            ...
            this.selectedPerSemester[this.semester].forEach(subject => {
                ...
                if (grade > 54) {
                    currentApprovedCreditsStandard += subject.getStandardCredits()
                    currentApprovedCreditsSCT += subject.getSCTCredits()
                }
                currentSemesterSumStandard = currentSemesterSumStandard + grade * subject.getStandardCredits()
            ...
            let resultSCT = 0
            if (this.totalCredits['Standard'] !== 0) {
                resultStandard = 100 * (currentSemesterSumStandard / (14 * Math.pow(this.semester, 1.06))) * currentApprovedCreditsStandard / this.totalCredits['Standard'] * fae
                resultStandard = Math.round(resultStandard * 100) / 100.0
            ...
            // this.card.select('.resStandard').text(resultStandard)
            this.card.select('.resSCT').text(resultSCT)
            // save results
            this.subjectGrades[this.semester] = semesterGrades
            this.currentSemesterSum["Standard"] = currentSemesterSumStandard
            this.currentSemesterSum["SCT"] = currentSemesterSumSCT
            this.faes[this.semester] = fae
            this.totalApprovedCredits["Standard"][this.semester] = currentApprovedCreditsStandard
